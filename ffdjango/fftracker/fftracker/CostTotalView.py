...
class MealPlanReportView(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        new_queryset = get_meals(queryset)
        serializer = MealPlanReportSerializer(new_queryset, many=True)
        return Response(serializer.data)

class CostTotalView(viewsets.ViewSet):
    def list(self, request):
        queryset_ingredients = Ingredients.objects.filter('in_date' > date.today().date())
        ##queryset_meal = Packaging.objects.filter(m_date__gt > date.today().date())
        ##for i in range(len(queryset_ingredients)):
        ##    for j in range(len(queryset_meals)):
        ##        print('looping here')
            
        return queryset_ingredients